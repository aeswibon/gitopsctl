...
BIN="$REPO_ROOT/.tmp_gitopsctl_cli_real_bin"
...
(curl -N "$API_URL/api/v1/events" --max-time 6 >"$TESTROOT/sse.actual.out" 2>/dev/null) &
CPID=$!
sleep 1

./gitopsctl --api-url "$API_URL" sync-app -n "$APP_NAME" >/dev/null
./gitopsctl --api-url "$API_URL" check-cluster -n "$CLUSTER_NAME" >/dev/null
